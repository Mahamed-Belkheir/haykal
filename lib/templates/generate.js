...
export class ${fName(name)}Model extends BaseEntityModel<${fName(name)}> implements ${fName(name)}ModelInterface {
    model = ${fName(name)}ObjectionModel
}`

// console.log(module.exports.model("user", {name: "string", age: "number"}))

module.exports.controller = (name) => `import {${fName(name)}ModelInterface} from "../models/interface/${name};
