...
module.exports = function initializeProject(projectName) {
    ...
    fs.writeFileSync(path.join(projectDir, "tsconfig.json"),  tsconfig()), { flag: "w" };
    fs.writeFileSync(path.join(projectDir, "knexfile.js"),  knex()), { flag: "w" };

    let pack = fs.readFileSync(path.join(projectDir, "package.json"), {encoding: "utf-8"});
    pack = pack.replace(`&& exit 1"`,
`&& exit 1",
    "watch": "nodemon dist/application/express/index.js",
    "dev": "tsc -w"`)
        console.log(pack)
      fs.writeFileSync(path.join(projectDir, "package.json"), pack, { flag: "w+" });
